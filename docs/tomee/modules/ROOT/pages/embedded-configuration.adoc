...
or the newer `EJBContainer.createEJBContainer()` API part of EJB 3.1 there is quite a bit of flexibility to how things are configured.
...
Here they are in order of preference; 1 = highest, 5 = lowest.
...
You can put 100% of your configuration in a
...
Everything you can configure via an openejb.xml (minus the element) can be configured/overridden via properties.
See xref:configuring-containers-in-tests.adoc[Configuring Containers in Tests] and xref:configuring-datasources-in-tests.adoc[Configuring DataSources in Tests].
...
See
xref:properties-listing.adoc[OpenEJB Properties] for details on those.
