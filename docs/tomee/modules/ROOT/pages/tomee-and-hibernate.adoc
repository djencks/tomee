...
`persistence.xml` file, which can be at any of the following locations in a webapp
...
There are many guides on how to use 2nd level cache with JPA 2. You can remove OpenJPA jar so less jars are in `<tomee-home>/lib` folder.
