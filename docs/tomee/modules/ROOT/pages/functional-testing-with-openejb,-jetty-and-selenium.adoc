...
There's also some sample code demonstrating this, available
...
This class sets up an embedded instance of Jetty, running on port 9091. You'll notice the setupJndi() method.
...
- this is to perform dependency injection.
...
http://people.apache.org/~jgallimore/PersonApp.zip[here] . You'll need Maven to build it, and you can build it and run the tests by running 'mvn clean install'.
