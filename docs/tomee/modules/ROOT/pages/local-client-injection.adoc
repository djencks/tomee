...
All EJB modules are scanned for `@LocalClient` classes, even if those EJB Modules are inside .war files as with the
xref:collapsed-ear.adoc[Collapsed EAR] . As well any modules that contain a META-INF/application-client.xml file will be scanned for @LocalClient classes.
...
Ensure that class was annotated with `@org`.apache.openejb.api.LocalClient and was successfully discovered and deployed. \{panel}
