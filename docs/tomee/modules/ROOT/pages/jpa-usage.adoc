...
See xref:containers-and-resources.adoc[Containers and Resources] for how to configure 'JtaManaged' and a full list of properties for DataSources.
...
xref:transaction-annotations.adoc[configure them otherwise] , or b) the caller already has a transaction in progress when it calls the bean.
...
reattach your object.
1. Use PersistenceContextType.EXTENDED as in '@PersistenceContext(unitName = "movie-unit", type = PersistenceContextType.EXTENDED)' for EntityManager refs instead of the default of PersistenceContextType.TRANSACTION. 1. If testing, use a technique to execute transactions in your test code.
That's described here in xref:{common-vc}::unit-testing-transactions.adoc[Unit testing transactions]
