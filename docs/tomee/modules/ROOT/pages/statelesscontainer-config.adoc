...
The properties can also be used when embedding TomEE via the `javax.ejb.embeddable.EJBContainer` API or
`InitialContext`
...
The `CloseTimeout` specifies the maximum time to wait for the pool to close and `PostConstruct`
...
Instances garbage collected will have their `@PreDestroy`
...
The setting should be increased to get the most out of the
`GarbageCollection` feature of the pool.
...
Using the same example `MinSize` of 4 and `MaxAge`
of `100 years`, the life spans of these initial four instances would be 100, 75, 50, and 25 years respectively.
...
Replacement is done in a background queue using the number of threads specified by
`CallbackThreads`.
...
The pool size will never grow beyond the the set `MaxSize`
value.
