...
//Not in common
TomEE relies on Apache CXF for JAX-RS (RESTful Services) and JAX-WS (Web Services).
It does not provide all CXF modules, but the most common ones for both specifications (JAX-RS is part of all distributions but JAX-WS is only part of plus one).
...
For features and interceptors the rule is the same: value is a list comma separated.
Each value of the list is either a qualified class name or an id of a service in resources.xml.
...
xref:developer/json/index.adoc[JAX-RS JSON] page shows a sample dedicated to JAX-RS.
